...
def gen_answer(model, tokenizer, prompt):
    ...
    result = pipe(f'''<|start_header_id|>system<|end_header_id|>You are a researcher writing an article 
    <|eot_id|><|start_header_id|>user<|end_header_id|> {prompt}<|eot_id|>''')
    return result[0]['generated_text'].split("<|eot_id|>", 3)[-1]
